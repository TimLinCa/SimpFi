...
const AddExpensePage: React.FC<AddExpensePageProps> = ({
        ...
        try {
          if (inputExpenseType === "personal") {
            ...
            if (groupExpenseDetail) {
              ...
              // Set items
              const formattedItems = groupExpenseDetail.userSplit.map(
                (item) => ({
                  id: item.id,
                  name: item.name,
                  amount: item.amount,
  ...
  const handleSave = async (): Promise<void> => {
    ...
    if (!title.trim()) {
      Alert.alert(
        "Missing Information",
        "Please enter a title for this expense"
      );
      return;
    }
    ...
    if (items.length === 0) {
      Alert.alert(
        "Missing Items",
        "Please add at least one item to your expense"
      );
      return;
    }
    ...
    try {
      ...
      if (expenseType === "personal") {
        ...
        if (group == null) {
          Alert.alert(
            "Missing Information",
            "Please select a group for this expense"
          );
          return;
        }
      ...
      if (expenseResult) {
        ...
        Alert.alert("Success", "Your expense has been saved successfully", [
      ...
      Alert.alert(
        "Error",
        "An unexpected error occurred while saving your expense."
      );
    }
  };
  ...
  function fetchGroupData() {
    ...
    if (selectedGroup.members && selectedGroup.members.length > 0) {
      processedMembers = selectedGroup.members.map((member) => ({
        ...
        name: member.name,
        avatar:
          member.avatar || "https://randomuser.me/api/portraits/men/1.jpg",
        selected: true,
        email: member.email,
  ...
  function generateExpenseMemberForExpense(
    itemAmount: number,
