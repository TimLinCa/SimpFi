...
export const addOrUpdatePersonalExpense = async (
    ...
    try {
        ...
        const { data, error } = await supabase.rpc('add_or_update_personal_expense', {
            p_user_id: userId,
            p_title: title,
...
export const getPersonalExpensesByMonth = async (
    ...
    try {
        ...
        const { data, error } = await supabase.rpc('get_personal_expenses_by_month', {
            user_uuid: userId,
            month: month,
            year: year
        });
        ...
        const expenses: PersonalExpense[] = (data || []).map((expense: { id: any; title: any; description: any; date: any; total_amount: number; category: Category; created_at: string }) => ({
            id: expense.id,
...
export async function getGroupExpenseById(expenseId: string): Promise<GroupDetailExpense> {
...
export function get_group_expense_by_id_for_user(expenseId: string, userId: string): Promise<GroupDetailExpense> {
    return new Promise(async (resolve, reject) => {
        try {
            const { data, error } = await supabase.rpc('get_group_expense_by_id_for_user', {
                expense_id: expenseId,
